...
def load_config():
    ...
    default_config = os.path.join(config_dir, 'default.yml')
    pc_name = platform.node()
    pc_specific_default_config = os.path.join(config_dir, f'{pc_name}.yml')
    default_configs = [default_config, pc_specific_default_config]
    ...
    parser.add_argument(
        '--input-type',
        choices=['kindle', 'clippings', 'list'],
        required=True,
        help='kindle - kindle db file (usually vocab.db); '
             'clippings - kindle "My Clippings.txt file" '
             'list - comma-separated list of words')
    parser.add_argument(
        '--input-file-path',
        help='path to input file',
        required=True)
    parser.add_argument(
        '--max-clippings-length',
    ...
    parser.add_argument(
        '--lang-dict-api-id',
        help='API ID. Required for Oxford Dictionary API')
    parser.add_argument(
        '--lang-dict-api-key',
        help='API ID. Required for Oxford Dictionary API')
    parser.add_argument(
        '--lang-dict-email',
        help='LinguaLeo account email/login')
    parser.add_argument(
        '--lang-dict-password',
        help='LinguaLeo account password')
    parser.add_argument(
        '--traditional-chinese',
        default=False,
    ...
    parser.add_argument(
        ...
        choices=['csv', 'anki'],
        required=True,
        help='csv - csv file with definitions; '
             'anki - write directly to anki file')
    parser.add_argument(
        '--out-file-path',
        help='path of the CSV output file')
    parser.add_argument(
        '--profile',
        help='Profile name of Anki. Will be ignored if --collection is provided.')
    parser.add_argument(
        '--collection',
        help='Path to Anki collection file (.anki file). '
...
def construct_collection_path(config):
    ...
    is_mac = platform_name.startswith("darwin")
    is_win = platform_name.startswith("win32")
    # TODO: check cygwin
    if is_win:
